...
class BillingCreateForm extends React.Component{
    ...
    calculateAmount(cptId){
        const cptAount = this.props.cpts[cptId].billed_amount;
        const unit = this.state.units ? this.state.units : 1;
        // debugger
        this.setState({
            amount: cptAount * unit
        })
    }
    ...
    findCode(searchKey, codeList){
        ...
        return codes.map(code => 
            ...
            <li key={code.id} className="" onClick={()=>{this.setState({cpt_id: code.cpt_code}),this.calculateAmount(code.id)}}> 
                {code.cpt_code}
                {/* {code.cpt_description} */}
            </li> : codeFilter[code.icd_code] ?
            <li key={code.id} className="" onClick={() => this.addToIcdList(code.icd_code)}> 
                {code.icd_code}
    ...
    handleSubmit(e){
        ...
        const cpt = document.getElementById('cpt-input') ? document.getElementById('cpt-input').value : null
        formData.append("billing_cpt[cpt_id]", cpt);
        ...
        const tempIcd = (
            <div>
                <div>
                    ...
                    <span onClick={()=>document.getElementById("icd1-input").value=null}>&times;</span>
                </div>

                <div>
                    <input type="text" id="icd2-input" placeholder="ICD 2" readOnly/>
                    <span onClick={()=>document.getElementById("icd2-input").value=null}>&times;</span>
                </div>

                <div>
                    <input type="text" id="icd3-input" placeholder="ICD 3" readOnly/>
                    <span onClick={()=>document.getElementById("icd3-input").value=null}>&times;</span>
                </div>

                <div>
                    <input type="text" id="icd4-input" placeholder="ICD 4" readOnly/>
                    <span onClick={()=>document.getElementById("icd4-input").value=null}>&times;</span>
                </div>
            </div>
        )
        ...
        const cpt = (
            <div>
                <input 
                    ...
                    onChange={this.updateBilling("cpt_id")}
                    readOnly
                />
            ...
            <form onSubmit={this.handleSubmit} className="">
                ...
                <div>
                    ...
                    <input 
                        ...
                        className=""
                        placeholder="unit"
                        onChange={this.updateBilling("units")}
